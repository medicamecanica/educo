...
//filter by student
 ...
 $sql .= " AND a.elementtype='educo_educostudent' AND a.fk_element=$id ";
...
if ($resql) {
     ...
     */
    $title = $langs->trans("ListOfObservations");
    ...
    dol_fiche_head($head, 'observations', $langs->trans('Student'), 0, 'student@educo');
    ...
    $nav = '';
    if ($optioncss != '')
        $nav .= '<input type="hidden" name="optioncss" value="' . $optioncss . '">';
    if ($actioncode)
    ...
    if ($user->rights->agenda->myactions->create || $user->rights->agenda->allactions->create) {
        ...
        $newparam .= 'id='.$id.'&&month=' . str_pad($month, 2, "0", STR_PAD_LEFT) . '&year=' . $tmpforcreatebutton['year'];
          $obsparam = '&status=0&actioncode=AC_OBS&origin=educo_educostudent&originid='.$id;
        //$param='month='.$monthshown.'&year='.$year;
        $hourminsec = '100000';
        $link = '<a class="butAction" href="' . DOL_URL_ROOT . '/comm/action/card.php?action=create&datep=' . sprintf("%04d%02d%02d", $tmpforcreatebutton['year'], $tmpforcreatebutton['mon'], $tmpforcreatebutton['mday']) . $hourminsec . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . ($newparam ? '?' . $newparam : '')) .$obsparam. '">';
        $link .= $langs->trans("NewObservation");
        $link .= '</a>';
    }
...
//    if (!empty($conf->global->AGENDA_SHOW_LINKED_OBJECT))
//        print_liste_field_titre("LinkedObject", $_SERVER["PHP_SELF"], "a.fk_element", $param, "", "", $sortfield, $sortorder);
    ...
    while ($i < min($num, $limit)) {
        ...
        if (!empty($conf->global->AGENDA_USE_EVENT_TYPE)) {
            if ($actionstatic->type_picto)
                ...
                if ($actionstatic->type_code == 'AC_TEL')
                    print img_picto('', 'object_phoning') . ' ';
                if ($actionstatic->type_code == 'AC_FAX')
                    print img_picto('', 'object_phoning_fax') . ' ';
                if ($actionstatic->type_code == 'AC_EMAIL')
