...
if ($resql) {
     ...
     */
    $title = $langs->trans("ListOfAbsences");
    ...
    dol_fiche_head($head, 'absences', $langs->trans('Student'), 0, 'student@educo');
    ...
    if ($user->rights->agenda->myactions->create || $user->rights->agenda->allactions->create) {
        ...
        $newparam .= '&id='.$id.'&month=' . str_pad($month, 2, "0", STR_PAD_LEFT) . '&year=' . $tmpforcreatebutton['year'];
         $absparam = '&&status=0&actioncode=AC_ABS&origin=educo_educostudent&originid='.$id;
        //$param='month='.$monthshown.'&year='.$year;
        $hourminsec = '100000';
        $link = '<a class="butAction" href="' . DOL_URL_ROOT . '/comm/action/card.php?action=create&datep=' . sprintf("%04d%02d%02d", $tmpforcreatebutton['year'], $tmpforcreatebutton['mon'], $tmpforcreatebutton['mday']) . $hourminsec . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . ($newparam ? '?' . $newparam : '')) .$absparam. '">';
        $link .= $langs->trans("NewAbsence");
        $link .= '</a>';
    }
