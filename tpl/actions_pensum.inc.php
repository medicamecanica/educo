...
if ($action == 'add' && !GETPOST('cancel')) {
    ...
    if (empty($object->level) && $object->level !== '0') {
        $error++;
        setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv("level")), null, 'errors');
    }
    if (empty($object->ref)) {
        $error++;
...
if ($action == 'confirm_delete') {
    ...
    if ($result > 0) {
        ...
        setEventMessages("RecordDeleted", null, 'mesgs');
         ...
         header("Location: " . dol_buildpath('/educo/academic/pensum.php?academicid='.$academicid, 1));
        // exit;
    } else {
        if (!empty($object->errors))
            setEventMessages(null, $object->errors, 'errors');
